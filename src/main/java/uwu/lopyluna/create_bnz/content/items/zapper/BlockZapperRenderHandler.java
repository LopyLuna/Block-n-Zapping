...
public class BlockZapperRenderHandler {
	...
	protected static void gatherSelectedBlocks() {
		...
		CompoundTag tagMain = heldMain.getOrCreateTag();
		CompoundTag tagOff = heldOff.getOrCreateTag();

		if (zapperInMain) if (!tagMain.contains("_Swap") || !zapperInOff) {
			createBrushOutline(tagMain, player, heldMain);
			return;
		}

		if (zapperInOff) {
			createBrushOutline(tagOff, player, heldOff);
			return;
		}
	...
	public static void createBrushOutline(CompoundTag tag, LocalPlayer player, ItemStack zapper) {
		...
		Vec3 range = player.getLookAngle().scale(zapperItem.getZappingRange(zapper));
